...
const Content = () => {
  ...
  const [uploadedFile, setUploaded] = useState<{ name: string; content: string } | null>(null);
...
${content.slice(0, 3000)}`
      ...
      setError("Failed to analyze schema. Please check your input and try again.");
      setState("error");
    }
  };
      ...
      <div className="relative z-10 p-8 w-full">
        ...
        <div className="flex items-start justify-between mb-8">
          <div>
            ...
            <p className="text-sm text-white/40 font-light">Upload a schema file and AI will break it down in plain English</p>
          </div>
          {state === "done" && (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-5">
            ...
            {inputMode === "file" ? (
              <>
                ...
                <div className="bg-white/3 border border-white/8 rounded-xl overflow-hidden">
                  <div className="px-4 py-3.5 border-b border-white/[0.07]">
                    <h2 className="text-sm font-semibold text-white/55" style={{ fontFamily: "'Syne', sans-serif" }}>From My Drive</h2>
                    <p className="text-[11px] text-white/25 mt-0.5">Select a schema file from your projects</p>
                  </div>
                  <div className="p-2 space-y-1">
                    {SAMPLE_FILES.map((f) => (
                      <button key={f.name} onClick={() => { setSelected(f.name); setUploaded(null); }}
                        ...
                        <div className="w-7 h-7 rounded-lg bg-blue-500/10 text-blue-400 flex items-center justify-center text-[9px] font-bold shrink-0">SQL</div>
                        <div className="flex-1 min-w-0">
                          <div className="text-xs font-medium text-white/70 truncate group-hover:text-white/90">{f.name}</div>
                        </div>
                        <span className={`text-[9px] font-medium border rounded-full px-1.5 py-0.5 shrink-0 ${ENGINE_STYLE[f.engine]}`}>{f.engine}</span>
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div className="bg-white/3 border border-white/8 rounded-xl overflow-hidden">
                  <div className="px-4 py-3.5 border-b border-white/[0.07]">
                    <h2 className="text-sm font-semibold text-white/55" style={{ fontFamily: "'Syne', sans-serif" }}>Upload a File</h2>
                    <p className="text-[11px] text-white/25 mt-0.5">Supports .sql .db .sqlite .dump</p>
                  </div>
                  <div className="p-4">
                    <input ref={fileRef} type="file" accept=".sql,.db,.sqlite,.dump,.txt" onChange={handleFileUpload} className="hidden" />
                      ...
                      {uploadedFile ? (
                        <>
                          <div className="w-10 h-10 rounded-xl bg-blue-500/10 border border-blue-500/25 flex items-center justify-center">
                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#60a5fa" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
                          </div>
                          <div className="text-sm font-medium text-blue-300">{uploadedFile.name}</div>
                          <div className="text-[11px] text-white/30">File ready for analysis</div>
                        </>
                      ) : (
                        <>
                          <div className="w-10 h-10 rounded-xl bg-white/4 group-hover:bg-blue-500/10 border border-white/8 group-hover:border-blue-500/25 flex items-center justify-center transition-all">
                            <svg xmlns="http://www.w3.org/2000/svg" width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-white/25 group-hover:text-blue-400 transition-colors"><path d="M4 14.899A7 7 0 1 1 15.71 8h1.79a4.5 4.5 0 0 1 2.5 8.242"/><path d="M12 12v9"/><path d="m16 16-4-4-4 4"/></svg>
                          </div>
                          <div className="text-sm text-white/35 group-hover:text-white/55 transition-colors">Drop your schema file here</div>
                          <div className="text-[11px] text-white/20">or click to browse</div>
                        </>
                      )}
                    </button>
                  </div>
              ...
              <div className="lg:col-span-2 bg-white/3 border border-white/8 rounded-xl overflow-hidden">
                <div className="px-4 py-3.5 border-b border-white/[0.07]">
                  <h2 className="text-sm font-semibold text-white/55" style={{ fontFamily: "'Syne', sans-serif" }}>Paste your SQL</h2>
                  <p className="text-[11px] text-white/25 mt-0.5">Paste CREATE TABLE statements or any SQL schema</p>
                </div>
                <div className="p-4">
                  <textarea
                    value={pastedSQL}
        ...
        {state !== "done" && (
          <div className="flex justify-center mb-8">
            <button
              onClick={analyze}
              disabled={state === "loading" || (!selectedFile && !uploadedFile && !pastedSQL.trim())}
          ...
          <div className="space-y-5">
            ...
            <div className="bg-white/3 border border-white/8 rounded-xl p-5">
              <div className="flex items-center gap-2 mb-3">
                ...
                {result.engine && <span className={`text-[9px] font-medium border rounded-full px-2 py-0.5 ml-auto ${ENGINE_STYLE[result.engine] ?? "bg-white/10 text-white/40 border-white/20"}`}>{result.engine}</span>}
              </div>
            ...
            <div className="bg-white/3 border border-white/8 rounded-xl overflow-hidden">
              <div className="px-5 py-4 border-b border-white/[0.07]">
                <h2 className="text-sm font-semibold text-white/60" style={{ fontFamily: "'Syne', sans-serif" }}>Tables ({result.tables?.length ?? 0})</h2>
              </div>
              <div className="divide-y divide-white/5">
                {result.tables?.map((t, i) => (
                  <div key={i} className="flex items-center gap-4 px-5 py-3 hover:bg-white/2 transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-xs font-semibold text-white/75 font-mono">{t.name}</div>
                      <div className="text-[11px] text-white/35 mt-0.5">{t.description}</div>
                    </div>
                    <div className="text-[10px] text-white/25 shrink-0">{t.columns} cols</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
              <div className="bg-white/3 border border-white/8 rounded-xl overflow-hidden">
                <div className="px-5 py-4 border-b border-white/[0.07]">
                  <h2 className="text-sm font-semibold text-white/60" style={{ fontFamily: "'Syne', sans-serif" }}>Relationships</h2>
                </div>
                <div className="p-4 space-y-2">
                  {result.relationships?.length ? result.relationships.map((r, i) => (
                    <div key={i} className="flex gap-2.5 text-xs text-white/50 leading-relaxed">
                      <span className="text-violet-400 mt-0.5 shrink-0">→</span>
                      {r}
                    </div>
                  )) : <p className="text-xs text-white/25">No relationships detected</p>}
                </div>
              </div>

              <div className="bg-white/3 border border-white/8 rounded-xl overflow-hidden">
                <div className="px-5 py-4 border-b border-white/[0.07]">
                  <h2 className="text-sm font-semibold text-white/60" style={{ fontFamily: "'Syne', sans-serif" }}>Potential Issues</h2>
                </div>
                <div className="p-4 space-y-2">
                  {result.issues?.length ? result.issues.map((issue, i) => (
                    <div key={i} className="flex gap-2.5 text-xs text-white/50 leading-relaxed">
                      <span className="text-amber-400 mt-0.5 shrink-0">⚠</span>
                      {issue}
                    </div>
                  )) : <p className="text-xs text-emerald-400/60">No issues detected 🎉</p>}
                </div>
              </div>
            </div>
            ...
            <div className="bg-white/3 border border-white/8 rounded-xl overflow-hidden">
              <div className="px-5 py-4 border-b border-white/[0.07]">
                <h2 className="text-sm font-semibold text-white/60" style={{ fontFamily: "'Syne', sans-serif" }}>AI Recommendations</h2>
              </div>
              <div className="p-4 space-y-2">
