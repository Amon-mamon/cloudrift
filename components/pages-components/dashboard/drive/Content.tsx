...
const PROJECT_COLOR_CLASS: Record<string, string> = {
  PostgreSQL: "bg-gradient-to-br from-sky-500/10 via-blue-500/10 to-violet-500/10 border-blue-500/20 text-blue-300",
  MySQL: "bg-gradient-to-br from-amber-500/10 via-orange-500/10 to-yellow-500/10 border-amber-500/20 text-amber-300",
  SQLite: "bg-gradient-to-br from-cyan-500/10 via-sky-500/10 to-blue-500/10 border-cyan-500/20 text-cyan-300",
};
...
const Content = () => {
  ...
  const filteredProjects = useMemo(() => {
    ...
    return projects.filter((project) => {
      ...
      const matchesFile = project.files.some((file) =>
  ...
  const loadDriveData = async () => {
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : "Failed to load drive data.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const loadShareRecipients = async (projectId?: string) => {
    ...
    try {
      ...
      setShareRecipients(
        Array.from(new Map(recipients.map((recipient) => [recipient.email, recipient])).values())
      );
    } catch {
      setShareRecipients([]);
    }
  ...
  const handleCreateProject = async () => {
    ...
    const engine = window.prompt("Database engine (PostgreSQL, MySQL, SQLite):", "PostgreSQL")?.trim() || "PostgreSQL";
    const payload = { name: name.trim(), engine, color: undefined };
    ...
    try {
      ...
      window.alert(caught instanceof Error ? caught.message : "Failed to create project.");
    }
  };
  ...
  const handleUploadFile = async () => {
    ...
    const validTypes = ["SQL", "DB", "CSV", "JSON", "DUMP", "ENV", "SQLITE", "OTHER"];
    ...
    if (!response.ok) {
      const json = await response.json().catch(() => null);
      window.alert(json?.error || "Could not upload file.");
      return;
    }
  ...
  const handleShareFile = async (fileId: string) => {
    ...
    if (openProject) {
      await loadShareRecipients(openProject.id);
    }
  };
      ...
      <div className="relative z-10 p-8 w-full">
        <div className="flex items-start justify-between mb-8">
          ...
          <div className="flex items-center gap-2">
            <button onClick={handleCreateProject} className="flex items-center gap-1.5 bg-transparent border border-white/12 hover:border-white/22 text-white/50 hover:text-white/75 text-sm font-medium px-4 py-2.5 rounded-xl transition-all">
            ...
            <button onClick={handleUploadFile} className="flex items-center gap-1.5 bg-blue-600 hover:bg-blue-500 transition-all hover:-translate-y-0.5 text-white text-sm font-medium px-4 py-2.5 rounded-xl">
        ...
        {openProject ? (
          <div>
            <div className="flex items-center gap-2 mb-5">
              <button onClick={handleCloseProject} className="flex items-center gap-1.5 text-xs text-white/40 hover:text-white/70 transition-colors">
              ...
              <span className="text-xs text-[#f0f4fa] font-medium">{openProject.name}</span>
            </div>

            <div className="bg-white/3 border border-white/8 rounded-xl p-5 mb-5">
              <div className="flex items-start justify-between flex-wrap gap-4">
                <div className="flex items-center gap-4">
                  <div className={`w-12 h-12 rounded-xl border flex items-center justify-center ${getProjectColorClass(openProject.engine)}`}>
                    <FolderIcon />
                  </div>
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className={`text-[10px] font-medium border rounded-full px-2 py-0.5 ${ENGINE_STYLE[openProject.engine ?? "UNKNOWN"]}`}>
                        {openProject.engine ?? "Unknown"}
                      </span>
                    </div>
                    <p className="text-xs text-white/35">{openProject.files.length} files · Modified {formatDate(openProject.updatedAt)}</p>
                  </div>
                </div>

                <div className="flex flex-col items-end gap-3">
                  <div className="text-right">
                    <p className="text-[10px] text-white/25 mb-1.5">Shared with</p>
                    <div className="flex items-center justify-end">
                      {shareRecipients.length > 0 ? (
                        <>
                          {shareRecipients.slice(0, 3).map((recipient, i) => (
                            <div
                              key={recipient.email}
                        ...
                        <span className="text-[10px] text-white/30">No recipients yet</span>
                      )}
                    </div>
                  </div>
                  <button onClick={handleUploadFile} className="flex items-center gap-1 text-[11px] text-blue-400 hover:text-blue-300 bg-blue-500/8 hover:bg-blue-500/[0.14] border border-blue-500/2 rounded-lg px-2.5 py-1 transition-all">
            ...
            <div className="bg-white/3 border border-white/8 rounded-xl overflow-hidden">
              <div className="grid grid-cols-[2fr_70px_100px_120px_100px_80px] gap-3 px-5 py-2.5 border-b border-white/5">
                {['Name', 'Type', 'Engine', 'Modified', 'Size', ''].map((heading) => (
                  <span key={heading} className="text-[10px] font-medium text-white/20 uppercase tracking-wider">{heading}</span>
                ))}
              </div>
              {openProject.files.map((file) => (
                <div key={file.id} className="grid grid-cols-[2fr_70px_100px_120px_100px_80px] gap-3 items-center px-5 py-3 hover:bg-white/3 border-b border-white/5 last:border-0 transition-colors group">
                  ...
                  <span className="text-[11px]">
                    {file.engine ? (
                      <span className={`text-[9px] font-medium border rounded-full px-1.5 py-0.5 ${ENGINE_STYLE[file.engine] ?? ENGINE_STYLE.UNKNOWN}`}>{file.engine}</span>
                    ) : (
                      <span className="text-white/20">—</span>
                    )}
                  </span>
                  <span className="text-[11px] text-white/30">{formatDate(file.updatedAt)}</span>
                  <span className="text-[11px] text-white/30">{getFileSizeLabel(file.sizeBytes)}</span>
                  <div className="hidden group-hover:flex items-center gap-1.5 justify-end">
                    <button onClick={() => handleShareFile(file.id)} type="button" className="w-6 h-6 flex items-center justify-center rounded-md bg-white/6 hover:bg-white/10 text-white/35 hover:text-white/65 transition-all">
                      <svg xmlns="http://www.w3.org/2000/svg" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" x2="15.42" y1="13.51" y2="17.49"/><line x1="15.41" x2="8.59" y1="6.51" y2="10.49"/></svg>
                    </button>
                    <button onClick={() => handleDeleteFile(file.id)} type="button" className="w-6 h-6 flex items-center justify-center rounded-md bg-red-500/8 hover:bg-red-500/15 text-red-400/50 hover:text-red-400 transition-all">
          ...
          <>
            {isLoading ? (
              <div className="rounded-xl border border-white/10 bg-white/5 p-8 text-center text-sm text-white/60">Loading your drive…</div>
            ) : projects.length === 0 ? (
              <div className="rounded-xl border border-dashed border-white/10 bg-white/5 p-8 text-center text-sm text-white/60">
                No projects found. Create a new project to start storing database files.
              </div>
            ) : view === "grid" ? (
                ...
                {filteredProjects.map((project) => (
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div className={`w-10 h-10 rounded-xl border flex items-center justify-center transition-all ${getProjectColorClass(project.engine)}`}>
                        <FolderIcon />
                      </div>
                      <div className="flex items-center">
                        {project.files.slice(0, 3).map((file, index) => (
                          <div key={file.id} title={file.name} className="w-6 h-6 rounded-full bg-white/10 border border-white/10 flex items-center justify-center text-[9px] text-white/40 font-bold shrink-0 -ml-1.5 first:ml-0 overflow-hidden text-ellipsis whitespace-nowrap">
                    ...
                    <div className="text-[11px] text-white/30 mb-2">
                      {project.files.length} files · {formatDate(project.updatedAt)}
                    </div>

                    <div className="mt-3 pt-3 border-t border-white/6 flex items-center justify-between">
                      <div className="flex gap-1">
                        <span className={`text-[9px] font-medium border rounded-full px-2 py-0.5 ${ENGINE_STYLE[project.engine ?? "UNKNOWN"]}`}>
                          {project.engine ?? "Unknown"}
                        </span>
                        {project.files.slice(0, 2).map((file) => (
                          <span key={file.id} className={`text-[9px] px-1.5 py-0.5 rounded font-medium ${FILE_ICON[file.type] ?? FILE_ICON.OTHER} bg-white/4`}>
                            {file.type}
                          </span>
                        ))}
                      </div>
              ...
              <div className="bg-white/3 border border-white/8 rounded-xl overflow-hidden">
                <div className="grid grid-cols-[2fr_120px_100px_120px_1fr_60px] gap-4 px-5 py-3 border-b border-white/[0.07]">
                  {["Project", "Engine", "Files", "Modified", "Members", ""].map((heading) => (
                    <span key={heading} className="text-[10px] font-medium text-white/20 uppercase tracking-wider">{heading}</span>
                  ))}
                </div>
                {filteredProjects.map((project) => (
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      <div className={`w-8 h-8 rounded-lg border flex items-center justify-center shrink-0 ${getProjectColorClass(project.engine)}`}>
                    ...
                    <span className={`text-[10px] font-medium border rounded-full px-2 py-0.5 w-fit ${ENGINE_STYLE[project.engine ?? "UNKNOWN"]}`}>
                      {project.engine ?? "Unknown"}
                    </span>
                    <span className="text-xs text-white/30">{project.files.length} files</span>
                    <span className="text-xs text-white/30">{formatDate(project.updatedAt)}</span>
                    <div className="flex items-center">
                      {project.files.slice(0, 4).map((file) => (
                        <div key={file.id} title={file.name} className="w-6 h-6 rounded-full bg-white/8 border-2 border-[#0a0f1c] flex items-center justify-center text-[9px] text-white/40 -ml-1.5 first:ml-0 overflow-hidden text-ellipsis whitespace-nowrap">
                          {file.name.slice(0, 2).toUpperCase()}
                        </div>
                      ))}
