"use client";

import { useEffect, useMemo, useState } from "react";
import { authFetch } from "@/lib/auth-fetch";

interface SharedFileData {
  id: string;
  name: string;
  type: string;
  engine?: string | null;
  aiAnalyzed: boolean;
  sizeBytes?: number | null;
  project?: { id: string; name: string } | null;
  updatedAt?: string | null;
}

interface SharedOwner {
  firstName: string;
  lastName: string;
  email: string;
}

interface ShareItem {
  id: string;
  permission: "FULL_ACCESS" | "READ_ONLY";
  createdAt: string;
  owner: SharedOwner;
  file: SharedFileData | null;
}

const FILE_ICON: Record<string, string> = {
  SQL: "bg-blue-500/10 text-blue-400",
  DB: "bg-violet-500/10 text-violet-400",
  CSV: "bg-emerald-500/10 text-emerald-400",
  JSON: "bg-amber-500/10 text-amber-400",
  DUMP: "bg-red-500/10 text-red-400",
  ENV: "bg-zinc-500/10 text-zinc-400",
  SQLITE: "bg-cyan-500/10 text-cyan-400",
  OTHER: "bg-white/10 text-white/50",
};

const ENGINE_STYLE: Record<string, string> = {
  PostgreSQL: "bg-blue-500/10 text-blue-300 border-blue-500/20",
  MySQL: "bg-amber-500/10 text-amber-300 border-amber-500/20",
  SQLite: "bg-cyan-500/10 text-cyan-300 border-cyan-500/20",
  UNKNOWN: "bg-white/10 text-white/40 border-white/10",
};

const PERMISSION_STYLE: Record<string, string> = {
  FULL_ACCESS: "bg-amber-500/10 text-amber-400 border-amber-500/20",
  READ_ONLY: "bg-blue-500/10 text-blue-300 border-blue-500/20",
};

const PERSON_COLORS = [
  "bg-blue-600",
  "bg-amber-600",
  "bg-violet-600",
  "bg-red-600",
  "bg-emerald-600",
  "bg-pink-600",
];

const formatDate = (value?: string | null) => {
  if (!value) return "Unknown";
  return new Date(value).toLocaleDateString(undefined, {
    month: "short",
    day: "numeric",
    year: "numeric",
  });
};

const getInitials = (firstName: string, lastName: string) => {
  return `${firstName?.[0] ?? ""}${lastName?.[0] ?? ""}`.toUpperCase();
};

const getFileSizeLabel = (size?: number | string | null) => {
  const parsed = typeof size === "string" ? Number(size) : size;
  if (!parsed || parsed <= 0) return "--";
  if (parsed < 1024) return `${parsed} B`;
  if (parsed < 1024 * 1024) return `${Math.round(parsed / 1024)} KB`;
  return `${Math.round(parsed / (1024 * 1024))} MB`;
};

const Content = () => {
  const [shares, setShares] = useState<ShareItem[]>([]);
  const [tab, setTab]       = useState<"files" | "people">("files");
  const [search, setSearch] = useState("");
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const loadSharedData = async () => {
    setIsLoading(true);
    setError(null);

    try {
      const response = await authFetch("/api/shared");
      if (!response.ok) {
        const json = await response.json().catch(() => null);
        throw new Error(json?.error || "Could not load shared files.");
      }

      const data = await response.json();
      setShares(data.shares ?? []);
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "Failed to load shared files.");
    } finally {
      setIsLoading(false);
    }
  };

  useEffect(() => {
    loadSharedData();
  }, []);

  const fileShares = useMemo(() => {
    return shares.filter((share) => share.file !== null);
  }, [shares]);

  const filteredFiles = useMemo(() => {
    const query = search.trim().toLowerCase();
    if (!query) return fileShares;

    return fileShares.filter((share) => {
      const file = share.file!;
      return (
        file.name.toLowerCase().includes(query) ||
        (file.engine ?? "").toLowerCase().includes(query) ||
        (file.project?.name ?? "").toLowerCase().includes(query) ||
        `${share.owner.firstName} ${share.owner.lastName}`.toLowerCase().includes(query)
      );
    });
  }, [fileShares, search]);

  const sharedPeople = useMemo(() => {
    const map = new Map<string, { initials: string; name: string; color: string; fileCount: number; lastShared: string; permission: "Full access" | "Read only"; role: string }>();

    shares.forEach((share, index) => {
      const email = share.owner.email || `unknown-${index}`;
      const name = `${share.owner.firstName} ${share.owner.lastName}`.trim();
      const permission = share.permission === "FULL_ACCESS" ? "Full access" : "Read only";
      const existing = map.get(email);

      if (!existing) {
        map.set(email, {
          initials: getInitials(share.owner.firstName, share.owner.lastName),
          name: name || "Unknown user",
          color: PERSON_COLORS[map.size % PERSON_COLORS.length],
          fileCount: 1,
          lastShared: formatDate(share.createdAt),
          permission,
          role: "Collaborator",
        });
      } else {
        existing.fileCount += 1;
        if (new Date(share.createdAt) > new Date(existing.lastShared)) {
          existing.lastShared = formatDate(share.createdAt);
        }
      }
    });

    return Array.from(map.values()).filter((person) =>
      person.name.toLowerCase().includes(search.trim().toLowerCase()) ||
      person.role.toLowerCase().includes(search.trim().toLowerCase())
    );
  }, [shares, search]);

  const sharedFileCount = fileShares.length;
  const uniquePeopleCount = useMemo(() => new Set(shares.map((s) => s.owner.email)).size, [shares]);
  const fullAccessCount = fileShares.filter((share) => share.permission === "FULL_ACCESS").length;
  const aiCount = fileShares.filter((share) => share.file?.aiAnalyzed).length;

  return (
    <div className="min-h-screen bg-[#070b14] text-[#e8edf5] font-sans">
      <div className="pointer-events-none fixed inset-0 overflow-hidden z-0">
        <div className="absolute w-125 h-125 rounded-full opacity-20 blur-[90px] bg-blue-600" style={{ top: "-120px", left: "140px" }} />
        <div className="absolute w-90 h-90 rounded-full opacity-15 blur-[80px] bg-emerald-600" style={{ top: "60px", right: "-60px" }} />
        <div className="absolute w-65 h-65 rounded-full opacity-10 blur-[70px] bg-violet-700" style={{ bottom: "60px", left: "45%" }} />
        <div className="absolute inset-0" style={{ backgroundImage: "linear-gradient(rgba(56,138,221,0.05) 1px, transparent 1px), linear-gradient(90deg, rgba(56,138,221,0.05) 1px, transparent 1px)", backgroundSize: "48px 48px" }} />
      </div>

      <div className="relative z-10 p-8 w-full">
        <div className="flex items-start justify-between mb-8">
          <div>
            <h1 className="text-3xl font-extrabold text-[#f0f4fa] tracking-[-1px] mb-1" style={{ fontFamily: "'Syne', sans-serif" }}>
              Shared with Me
            </h1>
            <p className="text-sm text-white/40 font-light">
              {sharedFileCount} files shared by {uniquePeopleCount} people
            </p>
          </div>
        </div>

        {error && (
          <div className="mb-6 rounded-xl border border-red-500/20 bg-red-500/10 p-4 text-sm text-red-100">
            {error}
          </div>
        )}

        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {[
            {
              label: "DB files shared",
              value: sharedFileCount.toString(),
              icon: "bg-blue-500/10 text-blue-400",
              iconSvg: <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><ellipse cx="12" cy="5" rx="9" ry="3"/><path d="M3 5V19A9 3 0 0 0 21 19V5"/><path d="M3 12A9 3 0 0 0 21 12"/></svg>,
            },
            {
              label: "Team members",
              value: uniquePeopleCount.toString(),
              icon: "bg-violet-500/10 text-violet-400",
              iconSvg: <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>,
            },
            {
              label: "Full access",
              value: fullAccessCount.toString(),
              icon: "bg-amber-500/10 text-amber-400",
              iconSvg: <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>,
            },
            {
              label: "AI analyzed",
              value: aiCount.toString(),
              icon: "bg-emerald-500/10 text-emerald-400",
              iconSvg: <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m12 3-1.912 5.813a2 2 0 0 1-1.275 1.275L3 12l5.813 1.912a2 2 0 0 1 1.275 1.275L12 21l1.912-5.813a2 2 0 0 1 1.275-1.275L21 12l-5.813-1.912a2 2 0 0 1-1.275-1.275L12 3Z"/></svg>,
            },
          ].map((stat) => (
            <div key={stat.label} className="group bg-white/3 border border-white/8 hover:border-blue-500/30 hover:bg-blue-500/4 rounded-xl p-4 transition-all">
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center mb-3 ${stat.icon}`}>{stat.iconSvg}</div>
              <div className="text-2xl font-bold text-[#f0f4fa] mb-0.5" style={{ fontFamily: "'Syne', sans-serif" }}>{stat.value}</div>
              <div className="text-[11px] text-white/35">{stat.label}</div>
            </div>
          ))}
        </div>

        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center bg-white/3 border border-white/8 rounded-xl p-1 gap-1">
            {(["files", "people"] as const).map((value) => (
              <button
                key={value}
                onClick={() => { setTab(value); setSearch(""); }}
                className={`px-4 py-1.5 rounded-lg text-xs font-medium transition-all ${
                  tab === value
                    ? "bg-blue-500/15 text-blue-300 border border-blue-500/2"
                    : "text-white/35 hover:text-white/60"
                }`}
              >
                {value === "files" ? `DB Files (${sharedFileCount})` : `Team (${uniquePeopleCount})`}
              </button>
            ))}
          </div>

          <div className="relative">
            <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="absolute left-3 top-1/2 -translate-y-1/2 text-white/25"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>
            <input
              type="text"
              placeholder={tab === "files" ? "Search files, project, or owner..." : "Search by name or role..."}
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="bg-white/3 border border-white/8 rounded-xl pl-9 pr-4 py-2 text-xs text-[#e8edf5] placeholder-white/20 outline-none focus:border-blue-500/50 transition-colors w-56"
            />
          </div>
        </div>

        {tab === "files" ? (
          <div className="bg-white/3 border border-white/8 rounded-xl overflow-hidden">
            <div className="grid grid-cols-[2fr_100px_140px_120px_100px_100px_60px] gap-3 px-5 py-3 border-b border-white/[0.07]">
              {['Name', 'Type', 'Project', 'Shared by', 'Modified', 'Access', ''].map((heading) => (
                <span key={heading} className="text-[10px] font-medium text-white/20 uppercase tracking-wider">{heading}</span>
              ))}
            </div>

            {isLoading ? (
              <div className="p-8 text-center text-sm text-white/60">Loading shared files…</div>
            ) : filteredFiles.length === 0 ? (
              <div className="p-8 text-center text-sm text-white/60">No shared files found.</div>
            ) : filteredFiles.map((share) => {
              const file = share.file!;
              return (
                <div key={share.id} className="grid grid-cols-[2fr_100px_140px_120px_100px_100px_60px] gap-3 items-center px-5 py-3 hover:bg-white/3 border-b border-white/4 last:border-0 transition-colors group">
                  <div className="flex items-center gap-2.5 min-w-0">
                    <div className={`w-7 h-7 rounded-lg flex items-center justify-center text-[10px] font-bold shrink-0 ${FILE_ICON[file.type] ?? FILE_ICON.OTHER}`}>
                      {file.type}
                    </div>
                    <div className="min-w-0">
                      <div className="text-xs font-medium text-white/70 truncate group-hover:text-white/90 transition-colors">{file.name}</div>
                      <div className="text-[10px] text-white/30">{file.project?.name ?? "No project"}</div>
                    </div>
                  </div>
                  <span>
                    {file.engine
                      ? <span className={`text-[9px] font-medium border rounded-full px-2 py-0.5 ${ENGINE_STYLE[file.engine] ?? ENGINE_STYLE.UNKNOWN}`}>{file.engine}</span>
                      : <span className="text-[11px] text-white/20">—</span>
                    }
                  </span>
                  <span className="text-[11px] text-white/35 truncate">{file.project?.name ?? "—"}</span>
                  <span className="text-[11px] text-white/35 truncate">{`${share.owner.firstName} ${share.owner.lastName}`}</span>
                  <span className="text-[11px] text-white/30">{formatDate(file.updatedAt ?? share.createdAt)}</span>
                  <span className={`text-[9px] font-medium border rounded-full px-2 py-0.5 ${PERMISSION_STYLE[share.permission]}`}>{share.permission === "FULL_ACCESS" ? "Full access" : "Read only"}</span>
                  <div className="hidden group-hover:flex items-center justify-end">
                    <button className="w-6 h-6 flex items-center justify-center rounded-md bg-white/6 hover:bg-white/10 text-white/35 hover:text-white/65 transition-all" title="Open file">
                      <svg xmlns="http://www.w3.org/2000/svg" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 12h16"/><path d="M12 4l8 8-8 8"/></svg>
                    </button>
                  </div>
                </div>
              );
            })}
          </div>
        ) : (
          <div className="bg-white/3 border border-white/8 rounded-xl overflow-hidden">
            <div className="grid grid-cols-[2fr_130px_100px_140px_120px_60px] gap-3 px-5 py-3 border-b border-white/[0.07]">
              {['Team member', 'Role', 'Files', 'Last shared', 'Access', ''].map((heading) => (
                <span key={heading} className="text-[10px] font-medium text-white/20 uppercase tracking-wider">{heading}</span>
              ))}
            </div>

            {isLoading ? (
              <div className="p-8 text-center text-sm text-white/60">Loading collaborators…</div>
            ) : sharedPeople.length === 0 ? (
              <div className="p-8 text-center text-sm text-white/60">No shared collaborators found.</div>
            ) : sharedPeople.map((person) => (
              <div key={person.name} className="grid grid-cols-[2fr_130px_100px_140px_120px_60px] gap-3 items-center px-5 py-3.5 hover:bg-white/3 border-b border-white/4 last:border-0 transition-colors group">
                <div className="flex items-center gap-3 min-w-0">
                  <div className={`w-8 h-8 rounded-full ${person.color} flex items-center justify-center text-[10px] font-bold text-white shrink-0`}>
                    {person.initials}
                  </div>
                  <span className="text-sm font-medium text-white/70 group-hover:text-white/90 truncate transition-colors" style={{ fontFamily: "'Syne', sans-serif" }}>{person.name}</span>
                </div>
                <span className="text-[11px] text-white/35">{person.role}</span>
                <span className="text-[11px] text-white/30">{person.fileCount} files</span>
                <span className="text-[11px] text-white/30">{person.lastShared}</span>
                <span className={`text-[9px] font-medium border rounded-full px-2 py-0.5 ${person.permission === "Full access" ? "bg-amber-500/10 text-amber-400 border-amber-500/20" : "bg-blue-500/10 text-blue-300 border-blue-500/20"}`}>{person.permission}</span>
                <div className="hidden group-hover:flex items-center justify-end">
                  <button className="w-6 h-6 flex items-center justify-center rounded-md bg-red-500/8 hover:bg-red-500/15 text-red-400/50 hover:text-red-400 transition-all" title="Revoke access">
                    <svg xmlns="http://www.w3.org/2000/svg" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>
                  </button>
                </div>
              </div>
            ))}
          </div>
        )}
      </div>
    </div>
  );
};

export default Content;
