  ...
  update: {
    ...
    icon: (
      <svg
        ...
        strokeLinejoin="round"
      >
        <path d="M3 12a9 9 0 0 1 9-9V1l4 4-4 4V5a7 7 0 1 0 7 7h-2" />
...
const Content = () => {
  ...
  const handleRestore = async (fileId: string) => {
    ...
    try {
      ...
      setError(
        caught instanceof Error ? caught.message : "Failed to restore file.",
      );
    } finally {
      setIsRestoring(null);
    }
  ...
  const stats = useMemo(
    () =>
      ALL_ACTIONS.reduce(
        (acc, action) => {
          acc[action] = activity.filter(
            (entry) => entry.action.toLowerCase() === action,
          ).length;
          return acc;
        },
  ...
  return (
      ...
      <div className="relative z-10 p-8 w-full">
        ...
        <div className="grid grid-cols-3 lg:grid-cols-6 gap-3 mb-8">
          {ALL_ACTIONS.map((action) => {
            ...
            return (
              ...
              >
                <div
                  className={`w-7 h-7 rounded-lg flex items-center justify-center mb-2 border ${style.color}`}
                >
                  {style.icon}
                </div>
                <div
                  className="text-lg font-bold text-[#f0f4fa]"
                  style={{ fontFamily: "'Syne', sans-serif" }}
                >
                  {count}
                </div>
        ...
        <div className="bg-white/3 border border-white/8 rounded-xl overflow-hidden">
          <div className="grid grid-cols-[180px_2fr_140px_120px_100px_100px] gap-3 px-5 py-3 border-b border-white/[0.07]">
            {["User", "File & Project", "Action", "Time", "IP Address", ""].map(
              (h) => (
                <span
                  key={h}
          ...
          {filtered.length === 0 ? (
            ...
            paginatedActivity.map((log) => {
              ...
              return (
                <div
                  key={log.id}
                  className="grid grid-cols-[180px_2fr_140px_120px_100px_100px] gap-3 items-center px-5 py-3 hover:bg-white/3 border-b border-white/4 last:border-0 transition-colors group"
                >
                  ...
                  <div className="flex items-center gap-2.5 min-w-0">
                    <div
                      className={`w-6 h-6 rounded-md flex items-center justify-center text-[9px] font-bold shrink-0 ${FILE_ICON[String(log.file?.type ?? "SQL").toUpperCase()] ?? "bg-white/10 text-white/40"}`}
                    >
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-[10px] text-white/28 truncate">
                        {projectName}
                      </div>
                    </div>
                  </div>
                  ...
                  <div
                    className={`inline-flex items-center gap-1.5 text-[10px] font-medium border rounded-full px-2 py-1 w-fit ${style.color}`}
                  >
                    {style.icon}
                    {style.label}
                  </div>
    ...
    "
            ...
            <span className="text-xs text-white/30">entries</span>
          </div>
          <div className="text-xs text-white/30">
            Showing {(currentPage - 1) * itemsPerPage + 1}–
            {Math.min(currentPage * itemsPerPage, activity.length)} of{" "}
            {activity.length}
          </div>
      ...
      "
            ...
            ${
              currentPage === page
                ...
                : "bg-white/3 border border-white/8 text-white/40 hover:text-white/70 hover:bg-white/5"
            }
          `}
                >
                  {page}
